...
local function basicVerifyBlock(requiredFields, classification, direction)
  ...
  -- logger.info("     block:", utiles.BufferToHexString(block))
...
function decrypt(requiredFields, key, direction)
  logger.info({"  requiredFields.FRMPayload:", utiles.BufferToHexString(requiredFields.FRMPayload)})
  logger.info({"  direction:", utiles.BufferToHexString(direction)})
  ...
  for ind = 1, k do
    ...
    if err ~= nil then
      logger.error({"function <crypto.encrypt> aes-128-ecb failed,", err})
      return nil
    end

    -- logger.info(" Ai:", utiles.BufferToHexString(Ai), "key:", utiles.BufferToHexString(newKey))
    -- logger.info("   encrypt FRMPayload:", crypto.hex(cipher))

    cipher = crypto.hex(cipher)
    local Si = buffer:new(string.len(cipher) / 2)
    utiles.BufferFromHexString(Si, 1, cipher)

    -- logger.info("   Si 1:", utiles.BufferToHexString(Si))
    -- Si = utiles.BufferSlice(Si, 1, 16)
    -- logger.info("   Si 2:", utiles.BufferToHexString(Si))
  ...
  -- logger.info(" pld:", utiles.BufferToHexString(pld))
  -- logger.info(" S:", utiles.BufferToHexString(S))
  ...
  logger.info({"   decrypt out:", utiles.BufferToHexString(out)})
  return out
end
...
function micCalculator(requiredFields, key, direction)
  ...
  newKey = utiles.BufferSlice(newKey, 1, 16)
  logger.info({"   newKey:", utiles.BufferToHexString(newKey)})
  local aesval = aesCmac(newKey, cmacBlock)
  logger.info({"   aesval:", utiles.BufferToHexString(aesval)})
